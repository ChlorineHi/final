...
def page_zhipuai_rag(CatFeed, tavily_Opened):
    ...
    if st.button('👩‍🚀获取答案！'):
        if user_input:
            ...
            st.session_state.messages_rag.append(("😾胖橘", response))
...
def page_zhipuai_memory(temperatureChoicePage, ChoosingPrompt):
    ...
    # Input box for user question
    user_input = st.text_area(':sneezing_face:请将你的小鱼干放进去(问问吧):', '')
    ...
    if st.button(':man-juggling:获取答案！'):
        if user_input:
            ...
            # Add ZhipuAI response to chat history
            st.session_state.messages_memory.append(("😼胖橘", response))
...
def langgraphRAG ():
    ...
    if st.button('👩‍🚀获取答案！'):
        if user_input:
            ...
            # Add ZhipuAI response to chat history
            st.session_state.messages_rag.append(("😾胖橘", response["messages"][0]))
...
def page_using_mind():
    ...
    st.title(":orange[胖橘]:cat:大模型使用指南")
    st.divider()
    ...
    st.text("感谢您使用胖橘大模型，我们团队在此向您表示诚挚的感谢！")
    ...
    st.text("在这里,我首先感谢北京邮电大学的老师和助教为我们创建了这样的平台来丰富我们对于模型原生应用的求知,")
    st.text("让我们在新时代来临之时能够站稳脚跟,突破创新,为ChinaAi添砖加瓦。")
    st.text("当然了,我(Mr.Chlorine)特别感谢北京邮电大学团队的Ms.Chlore,Ms.Charlie")
    st.text("和Mr.Langongyao收集的数据集和协助。可以负责任的说,没有你们就没有这个作品的落地。")
    st.text("在此,我向你们致谢,愿后续我们可以继续合作,再造一堆'史山代码'。")
    st.text("以下是所有参与人员名单:")
    st.text("<<<开发团队(BUPT[北京邮电大学]):")
    st.text("---leader,programmer:  Mr.Chlorine")
    st.text("------team members,collector:  Ms.Charlie,Ms.Chloe,Mr.Langongyao")
    st.text("<<<医学顾问(CSU[中南大学.湘雅医学院])")
    st.text("---consultant:Ms.Chelsea")
    st.text("<<<测试人员:")
    st.text("---Mr.Moran(RUC[中国人民大学])")
    st.text("---Mr.LooseAnus(NWPU[西北工业大学])")
    st.text("在这里一并谢谢大家！")

    st.text("没有大家的热心打磨,齐心协力,就没有这个作品的实现与完善,再到上载。")
    st.text("最后我还要感谢我的舍友们(Mr.Harris,Mr.Rugosa,Mr.Dick),感谢他们一年的陪伴让我的大学生活")
    st.text("熠熠生辉,并有能力住进学五(孩子们,这很重要)。")
...
def developer_Cookbook():
    ...
    st.subheader(":orange[胖橘]使用多个函数进行修改,")
    st.subheader("更好的为您进行模型调优。")
    st.subheader("", divider='gray')
    st.subheader("⏰'拉杆'温度调节")
    st.text("在'铲屎官'页面中,我们开发了temperature组件为您进行")
...
def add_message_decorator(func):
    def wrapper(temp):
        ...
        if temp == 2.0:
            ...
            st.write("温度过低⛄️,胖橘冬眠。一觉醒来")
            st.write("最好的朋友在身边,最爱的人在对面")
            st.write("愿我们都能成为更好的自己。💪")
        else:
            st.write("继续拉动'滑杆'为胖橘调参吧🙌")
...
# Sec.Token修改
def TokenChoice():
    catFeed = st.text_input("请先修改胖橘😸的日常猫粮Tokens使用额度再询问问题！",
                            max_chars=55)
    if catFeed == 0:
        return 500
    st.button("😿提交你的胖橘猫粮额度!😼")
    return catFeed


# Thr.使用不同提示词进行记忆检索
def prompt_memorize():
    Looking = st.text_input("你对胖橘的最新期待(Prompt)")
    # 这里直接继承到Cookbook的方法
    return Looking
...
def main():
    ...
    def login():
        ...
        if role == "👨‍🎓铲屎官":
            extra_info = st.text_input("请输入您的姓名(请注意大小写)")
        ...
        st.divider()
        st.text("'投喂宝'面向普通用户,'铲屎官'面向开发者。")
        st.text("其中12位用户拥有查阅Cookbook并可以通过函数接口直接微调大模型的权限")
        st.text("请开发者选择'铲屎官'并在弹出的消息框中输入姓名,为了安全性,系统自动会给出")
    ...
    st.sidebar.title("🦹‍♂️:red[胖橘猫舍]")
    if st.session_state.role == "🕵️‍♂️投喂宝":
        page = st.sidebar.radio("📊请你选择使用的大模型",
                                ['胖橘RAG 智能检索大模型', '胖橘记忆检索大模型' ,
                                 '大模型使用指南'
                                 ])
    else:
        page = st.sidebar.radio("📊请你选择使用的大模型",
                                ['胖橘RAG 智能检索大模型', '胖橘记忆检索大模型','胖橘Agent' ,
    ...
    if st.session_state.role is None:
        ...
        if st.session_state.role == "🕵️‍♂️投喂宝":
            ...
            #   langgraphRAG()
                ...
                page_using_mind()
        elif st.session_state.role == "👨‍🎓铲屎官":
            if page == '胖橘RAG 智能检索大模型':
